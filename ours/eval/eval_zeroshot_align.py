...
class Pulse_Align():
    ...
    def __init__(self,
        ...
        if video_feature_path is None:
            video_feature_path = '/data/engs2527/pulsedata/features/'
        self.video_feature_path = video_feature_path

        anno_path = '/data/engs2527/pulsedata/' + source
        ...
        for i in self.anno.keys():
            assert os.path.exists(os.path.join(self.video_feature_path, i + ".npy"))
...
@torch.no_grad()
def test_alignment_pulse(get_text_visual_sim, device, args, video_feature_path=None, epoch=0):
    D = Pulse_Align(seq_len=-1, source='text_all_alignable_test5.json', video_feature_path=video_feature_path)
    ...
    save = (np.random.uniform() > 0.8) and (epoch > 400) # if save alignment matrix
    ...
    for input_data in tqdm(data_loader, total=len(data_loader)):
        ...
        if method == 'overlap-seq':
            ...
            for idx, step_ in enumerate(step):
                ...
                logits_ = get_text_visual_sim(video[:, step_:min(vlen, step_+seq_len)], active_text_str, 
                    save=save, abs_text_pos=active_abs_text_pos)
        ...
        ## sorting ##
        # prob = torch.sort(prob.argmax(-1), dim=-1)[0]  # [b,t](index) ->sort-> [b,t](ground truth) 

        ## viterbi ##
        # prob = prob.clone().detach().cpu().numpy()
        # prob = torch.from_numpy(np.resize(prob, (prob.shape[1], prob.shape[0]//10)))
        # prob = get_viterbi_gt(prob) * 10

        for text_idx in range(sim.size(0)):
            s = math.floor(start_idx_aligned[text_idx])
            e = math.ceil(end_idx_aligned[text_idx])
            # recall.append(s <= prob[text_idx].item() <= e)
...
if __name__ == '__main__':
    ...
    test_alignment_pulse(get_text_visual_sim, device, args, video_feature_path)
    sys.exit(0)
    ...
    # PulseNet features:
    # "overlap-seq": {'Recall': 0.290, 'AUC': 0.739}