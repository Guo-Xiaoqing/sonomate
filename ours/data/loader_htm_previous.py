...
inter = 6
dataset_path = '/data/engs2527/pulsedata/'
video_feature_path = '/data/engs2527/pulsedata/featuresAll/'
...
class HTM_FeatureLoader():
    def __init__(self,
        ...
        tag_to_asr = {
            'htm-pulseInter6': 'text_all_inter6.json',
            'htm-pulse': 'text_all.json'}
        ...
        num_val = min(int(len(all_vids) * 0.002), 1000) 
        if mode == 'train':
    ...
    def __getitem__(self, index):
        ...
        del array
        vlen = feature.size(0)
        vstart = int(path.split('_')[1])
        vend = int(path.split('_')[2].split('.')[0])
        assert vlen == (vend - vstart + 1)

        # sample text, then cut the video
        caps, (start_timestamp, end_timestamp) = self._get_text(vid, vstart, vend)
        # video_feature = self._get_video_feature(feature, vid, start_timestamp, end_timestamp)   ### original
        video_feature = self._get_video_feature(feature, vid, start_timestamp-vstart, end_timestamp-vstart)   ### revised
    ...
    def _get_text(self, vid, vstart, vend):
        ...
        cap_df = cap_df[cap_df['start'] >= vstart]
        cap_df = cap_df[cap_df['end'] <= vend]
        ...
        if (cap_df['start'] < last_timestamp - self.duration - 1).sum() == 0:
            ...
            start_idx = np.random.choice(
                # cap_df.index[cap_df['start'] < last_timestamp - self.duration])  ### original
                cap_df.index[cap_df['start'] < last_timestamp - self.duration]) - int(cap_df.index[0])  ### revised
