...
from data.loader_medvqa6 import Binary_VQA_Dataset, Binary_VQA_Dataset_test
import difflib 
import random 
...
def test_vqa_pulse(model, device, args):
    ...
    test_json_path = [
        '/media/engs2527/2TBFast/sonomate/vqa_dataset/VQA_dataset5/testImageQA_freeze.json',
        '/media/engs2527/2TBFast/sonomate/vqa_dataset/VQA_dataset5/testImageQA_trim.json',
        '/media/engs2527/2TBFast/sonomate/vqa_dataset/VQA_dataset5/testImageQA_day.json',
        '/media/engs2527/2TBFast/sonomate/vqa_dataset/VQA_dataset5/testImageQA_anatomy.json',
        '/media/engs2527/2TBFast/sonomate/vqa_dataset/VQA_dataset5/testImageQA_anatomy_open.json',
        '/media/engs2527/2TBFast/sonomate/vqa_dataset/VQA_dataset5/testImageQA_anatomy_openSource.json'
        ]
    task = ['freeze', 'trim', 'day', 'anatomy', 'anatomy_open', 'anatomy_openSource']
            ...
            if True:
                ...
                for idx, input_data in tqdm(enumerate(val_loader), total=len(val_loader)):
                    ...
                    for out in range(outputs.shape[0]):
                        ...
                        # generated_text1 = model.tokenizer.decoder(outputs1[out])
                        # generated_text1 = ' '.join(generated_text1).strip()

                        if is_blank:
                            if t == 'day' or t == 'anatomy_open' or t == 'anatomy_openSource':
                                cho = Choices[out][2:].replace(' b ', ' ').replace(' c ', ' ').replace(' d ', ' ').split('[SEP]')
                                index_label = find_most_similar_index(cho, Label[out].lower())
                                index_pred = find_most_similar_index(cho, generated_text)
                                # print(cho, Label[out], ',', generated_text, ',', index_label, index_pred)
                            else:
                                index_label = Label[out].lower()
                                index_pred = generated_text.lower()
                        else:
                            index_label = find_most_similar_index(Choice[out].split(' '), Answer[out])
                            index_pred = find_most_similar_index(Choice[out].split(' '), generated_text)
                
                        if index_pred == index_label:
                            ACC = ACC +1
                        cc = cc + 1
                    # import pdb;pdb.set_trace()
                    # print(Answer[out], generated_text, generated_text1)
                    # del outputs1
